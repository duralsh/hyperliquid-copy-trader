...
export function RightSidebar({ botStatus, onViewTrader, isSwitching }: Props) {
  ...
  if (!isAuthenticated) {
    return (
      <aside className="right-sidebar flex flex-col border-l border-border/30 bg-bg-secondary overflow-hidden">
        <div className="flex shrink-0 border-b border-border/30 items-center justify-between">
          <span className="px-4 py-2 text-xs font-bold tracking-wider text-text-dim">SESSION</span>
          <button
  ...
  if (!user?.onboarded) {
    return (
      <aside className="right-sidebar flex flex-col border-l border-border/30 bg-bg-secondary overflow-hidden">
        <div className="flex shrink-0 border-b border-border/30 items-center justify-between">
          <span className="px-4 py-2 text-xs font-bold tracking-wider text-amber">SETUP</span>
          <button
            onClick={() => setCollapsed(true)}
            title="Collapse sidebar"
            className="px-2 py-2 text-text-dim hover:text-amber transition-colors"
